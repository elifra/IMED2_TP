...
int main()
{
    ...
    for(int i = Hmin; i < Hmax; i++) {
        for(int j = Wmin; j < Wmax; j++) {
            DesiredFeatures[cptVector] = ImageD[i][j];
            cptVector++;
        }
    }
    ...
    try  {
        while(1)
        {
            ...
            object_velocity[3] = vpMath::rad(5)*sin((2*PI/5)*time); // rotation (angular velocity has to be provided in rad/s. Use vpMath::rad() for converting degree to rad)
            object_velocity[4] = vpMath::rad(5)*sin((2*PI/5)*time); // rotation (angular velocity has to be provided in rad/s. Use vpMath::rad() for converting degree to rad)
            object_velocity[5] = vpMath::rad(5)*sin((2*PI/5)*time); // rotation (angular velocity has to be provided in rad/s. Use vpMath::rad() for converting degree to rad)
            // Question 11
